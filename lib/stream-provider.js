import { get4KHDHubStreams, getHDHub4uStreams, getMKVCinemasStreams, getMalluMvStreams, getCineDozeStreams, getVixSrcStreams, getMoviesModStreams, getMoviesLeechStreams, getAnimeFlixStreams, get111477Streams, getXDMoviesStreams } from './http-streams.js';
...
import Relaygate from './relaygate.js';
...
import { filterSeason, filterEpisode, filterYear, matchesSeriesTitle, hasEpisodeMarker, normalizeTitle } from './util/filter-torrents.js';
import { getResolutionFromName, formatSize, getCodec, resolutionOrder, sizeToBytes, extractFileName, isSeriesLikeTitle, filterByYear as torrentFilterByYear } from './common/torrent-utils.js';
...
/** Last line of defense for Relaygate movies: SQLite cache can hold stale rows; filterYear() alone is too permissive. */
function relaygateMovieTorrentMatchesCinemeta(torrent, cinemetaDetails) {
  if (!cinemetaDetails) return true;
  const title = torrent?.name || torrent?.Title || torrent?.title || '';
  if (isSeriesLikeTitle(title)) return false;
  try {
    const parsed = PTT.parse(title) || {};
    if (parsed.season != null || parsed.seasons) return false;
  } catch {}
  if (!torrentFilterByYear({ ...torrent, name: title, Title: title }, cinemetaDetails, 'SP-RG')) return false;
  if (!cinemetaDetails.name) return true;
...
async function getMovieStreamsFromProvider(debridProvider, apiKey, type, id, config, cinemetaDetails, searchKey) {
  ...
  } else if (debridProvider == "Relaygate") {
    const torrents = await getCachedTorrents(
      debridProvider,
      type,
      id,
      config,
      () => Relaygate.searchRelaygateTorrents(apiKey, type, id, config),
      () => Relaygate.searchPersonalFiles(apiKey, searchKey, 0.3, config)
    );
    if (torrents && torrents.length) {
      return torrents.sort(sortTorrents)
        .filter(t => relaygateMovieTorrentMatchesCinemeta(t, cinemetaDetails))
        .filter(t => filterYear(t, cinemetaDetails))
        .map(td => toStream(td, type, providerConfig))
        .filter(Boolean);
    }
  ...
  if (!cinemetaFailed && config.DebridServices && Array.isArray(config.DebridServices) && config.DebridServices.length > 0) {
    config.DebridServices.forEach(service => {
      ...
      if (service.provider === 'Usenet') {
        ...
        const use111477 = service.http111477 === true;
        ...
        if (use111477) {
          addStreamTask(
            withTimeout(
              getCachedTorrents('httpstreaming', type, `${id}-111477`, config, () =>
                get111477Streams(id, type, null, null, config, cinemetaDetails)
              ).then(streams => wrapHttpStreamsWithResolver(streams, config.host)),
              getHttpStreamingTimeout('111477'),
              '111477'
            ),
            { provider: '111477', earlyReturnDisabled: false, earlyReturnTimeoutMs: earlyReturnMeta.earlyReturnTimeoutMs }
          );
        }
        ...
        const serviceConfig = {
          ...
          relaygateProviders: service.relaygateProviders,
          relaygateProvider: service.relaygateProvider
        };
    ...
    config.DebridServices.forEach(service => {
      ...
      if (service.provider === 'Usenet') {
        ...
        if (use111477) {
          addStreamTask(
            withTimeout(
              getCachedTorrents('httpstreaming', type, `${imdbId}-111477-${season}:${episode}`, config, () =>
                get111477Streams(imdbId, type, season, episode, config, cinemetaDetails)
              ).then(streams => wrapHttpStreamsWithResolver(streams, config.host)),
              getHttpStreamingTimeout('111477'),
              '111477'
            ),
        ...
        addStreamTask(
          (async () => {
            ...
            console.log(`[PARALLEL-SEARCH] 🚀 Starting ${serviceName} search at ${new Date(startTime).toISOString()}`);
            try {
              const results = await getSeriesStreamsFromProvider(service.provider, service.apiKey, type, id, serviceConfig, cinemetaDetails, searchKey, season, episode);
              const duration = Date.now() - startTime;
              console.log(`[PARALLEL-SEARCH] ✅ ${serviceName} completed in ${(duration/1000).toFixed(2)}s with ${results?.length || 0} results`);
              return results;
            } catch (error) {
  ...
  const performResolve = async () => {
    try {
      ...
      return resolvedUrl;
        ...
        return Relaygate.unrestrictUrl(apiKey, itemId, hostUrl, clientIp, config);
      } else {
