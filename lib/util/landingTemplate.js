	...
	const getDebridServices = () => {
		...
		rows.forEach(row => {
			...
			if (provider === 'Usenet') {
				...
				const http111477 = row.querySelector('.http-111477')?.checked ?? false;
				const httpXDMovies = row.querySelector('.http-xdmovies')?.checked ?? false;
			...
			} else if (provider === 'Relaygate') {
				const relaygateUrl = row.querySelector('.relaygate-url')?.value || '';
				const relaygateProviders = Array.from(row.querySelectorAll('.relaygate-provider-cb:checked')).map(cb => cb.value);
				if (apiKey) {
					services.push({
						provider,
						apiKey,
						relaygateUrl,
						relaygateProviders: relaygateProviders.length ? relaygateProviders : ['torbox']
					});
				}
...
const createServiceRow = (provider = '${process.env.DEFAULT_DEBRID_SERVICE || 'RealDebrid'}', apiKey = '', extraConfig = {}) => {
		...
		const allServices = [
			...
			{ value: 'Relaygate', label: 'Relaygate' },
		...
		const updateApiKeyLink = () => {
			...
			if (providerValue === 'Relaygate') {
				const raw = (configDiv.querySelector('.relaygate-url')?.value || '').trim();
				let origin = 'https://relaygate.xyz';
				if (raw) {
					try { origin = new URL(raw).origin; } catch (e) { origin = 'https://relaygate.xyz'; }
				}
				const proxyKeysUrl = origin + '/dashboard/proxy-keys';
				apiKeyLink.innerHTML = \`<a href="\${proxyKeysUrl}" target="_blank" rel="noopener noreferrer" style="color: #64ffda; text-decoration: none;">→ Get proxy API key</a>\`;
				return;
			}
			const apiLinks = {
		...
		const updateUsenetFields = () => {
			...
			const relaygateAdvancedWrap = configDiv.querySelector('.relaygate-advanced-wrap');
			const relaygateAdvancedBtn = configDiv.querySelector('.relaygate-advanced-toggle');
			const relaygateUrl = configDiv.querySelector('.relaygate-url');
			const relaygateProvidersMultiselect = configDiv.querySelector('.relaygate-providers-multiselect');
			const relaygateFooterHelp = configDiv.querySelector('.relaygate-footer-help');
			...
			if (relaygateAdvancedWrap) relaygateAdvancedWrap.remove();
			if (relaygateAdvancedBtn) relaygateAdvancedBtn.remove();
			if (relaygateUrl) relaygateUrl.remove();
			if (relaygateProvidersMultiselect) relaygateProvidersMultiselect.remove();
			if (relaygateFooterHelp) relaygateFooterHelp.remove();
			...
			if (select.value === 'HomeMedia') {
				...
				httpConfigDiv.innerHTML = \`<div style=\"font-weight: 600; margin-bottom: 0.5em; color: #64ffda; font-size: 0.9rem;\">HTTP Streaming Sources</div><div style=\"display: flex; flex-direction: column; gap: 0.5em;\"><label style=\"display: flex; align-items: center; font-size: 0.9rem; cursor: pointer;\"><input type=\"checkbox\" class=\"http-4khdhub\" checked style=\"margin-right: 8px;\">4KHDHub</label><label style=\"display: flex; align-items: center; font-size: 0.9rem; cursor: pointer;\"><input type=\"checkbox\" class=\"http-hdhub4u\" checked style=\"margin-right: 8px;\">HDHub4u</label><label style=\"display: flex; align-items: center; font-size: 0.9rem; cursor: pointer;\"><input type=\"checkbox\" class=\"http-uhdmovies\" checked style=\"margin-right: 8px;\">UHDMovies</label><label style=\"display: flex; align-items: center; font-size: 0.9rem; cursor: pointer;\"><input type=\"checkbox\" class=\"http-moviesdrive\" checked style=\"margin-right: 8px;\">MoviesDrive</label><label style=\"display: flex; align-items: center; font-size: 0.9rem; cursor: pointer;\"><input type=\"checkbox\" class=\"http-mkvcinemas\" checked style=\"margin-right: 8px;\">MKVCinemas</label><label style=\"display: flex; align-items: center; font-size: 0.9rem; cursor: pointer;\"><input type=\"checkbox\" class=\"http-mallumv\" checked style=\"margin-right: 8px;\">MalluMv</label><label style=\"display: flex; align-items: center; font-size: 0.9rem; cursor: pointer;\"><input type=\"checkbox\" class=\"http-cinedoze\" checked style=\"margin-right: 8px;\">CineDoze</label><label style=\"display: flex; align-items: center; font-size: 0.9rem; cursor: pointer;\"><input type=\"checkbox\" class=\"http-vixsrc\" checked style=\"margin-right: 8px;\">VixSrc (HLS)</label><label style=\"display: flex; align-items: center; font-size: 0.9rem; cursor: pointer;\"><input type=\"checkbox\" class=\"http-moviesmod\" checked style=\"margin-right: 8px;\">MoviesMod</label><label style=\"display: flex; align-items: center; font-size: 0.9rem; cursor: pointer;\"><input type=\"checkbox\" class=\"http-moviesleech\" checked style=\"margin-right: 8px;\">MoviesLeech</label><label style=\"display: flex; align-items: center; font-size: 0.9rem; cursor: pointer;\"><input type=\"checkbox\" class=\"http-animeflix\" checked style=\"margin-right: 8px;\">AnimeFlix</label><label style=\"display: flex; align-items: center; font-size: 0.9rem; cursor: pointer;\"><input type=\"checkbox\" class=\"http-111477\" style=\"margin-right: 8px;\">111477</label><label style=\"display: flex; align-items: center; font-size: 0.9rem; cursor: pointer;\"><input type=\"checkbox\" class=\"http-xdmovies\" style=\"margin-right: 8px;\">XDMovies (API)</label></div>\`;
				...
				const http111477Checkbox = httpConfigDiv.querySelector('.http-111477');
				...
				if (http111477Checkbox && extraConfig.http111477 !== undefined) http111477Checkbox.checked = extraConfig.http111477;
				...
				configDiv.appendChild(helpText);
			} else if (select.value === 'Relaygate') {
				input.type = 'password';
				input.autocomplete = 'off';
				input.placeholder = 'Relaygate proxy API key';
				const savedProviders = Array.isArray(extraConfig.relaygateProviders) ? extraConfig.relaygateProviders : (extraConfig.relaygateProvider ? [extraConfig.relaygateProvider] : ['torbox']);
				const slugList = [
					{ value: 'torbox', label: 'TorBox' },
					{ value: 'real_debrid', label: 'Real-Debrid' },
					{ value: 'alldebrid', label: 'AllDebrid' },
					{ value: 'premiumize', label: 'Premiumize' }
				];
				const multiselectWrap = document.createElement('div');
				multiselectWrap.className = 'relaygate-providers-multiselect';
				multiselectWrap.style.marginTop = '0.75em';
				const multiselectLabel = document.createElement('div');
				multiselectLabel.className = 'label-to-top';
				multiselectLabel.style.fontSize = '0.9rem';
				multiselectLabel.style.marginBottom = '0.35em';
				multiselectLabel.textContent = 'Debrid providers (vault-aligned)';
				multiselectWrap.appendChild(multiselectLabel);
				const grid = document.createElement('div');
				grid.style.display = 'grid';
				grid.style.gridTemplateColumns = '1fr 1fr';
				grid.style.gap = '0.4em 1em';
				slugList.forEach(({ value, label }) => {
					const lab = document.createElement('label');
					lab.style.display = 'flex';
					lab.style.alignItems = 'center';
					lab.style.fontSize = '0.88rem';
					lab.style.cursor = 'pointer';
					const cb = document.createElement('input');
					cb.type = 'checkbox';
					cb.className = 'relaygate-provider-cb';
					cb.value = value;
					cb.checked = savedProviders.includes(value);
					cb.style.marginRight = '8px';
					cb.addEventListener('change', debouncedUpdateLink);
					lab.appendChild(cb);
					lab.appendChild(document.createTextNode(label));
					grid.appendChild(lab);
				});
				multiselectWrap.appendChild(grid);
				apiKeyLink.after(multiselectWrap);

				const advancedBtn = document.createElement('button');
				advancedBtn.type = 'button';
				advancedBtn.className = 'relaygate-advanced-toggle';
				advancedBtn.textContent = 'Advanced options';
				advancedBtn.style.marginTop = '0.75em';
				advancedBtn.style.padding = '0.45em 0.9em';
				advancedBtn.style.fontSize = '0.88rem';
				advancedBtn.style.background = 'transparent';
				advancedBtn.style.border = '1px solid rgba(100, 255, 218, 0.35)';
				advancedBtn.style.color = '#64ffda';
				advancedBtn.style.borderRadius = '5px';
				advancedBtn.style.cursor = 'pointer';
				multiselectWrap.after(advancedBtn);

				const advancedWrap = document.createElement('div');
				advancedWrap.className = 'relaygate-advanced-wrap';
				advancedWrap.style.display = 'none';
				advancedWrap.style.marginTop = '0.5em';
				const relaygateUrlInput = document.createElement('input');
				relaygateUrlInput.type = 'text';
				relaygateUrlInput.className = 'relaygate-url full-width';
				relaygateUrlInput.placeholder = 'https://relaygate.example (optional Relaygate API base)';
				relaygateUrlInput.value = extraConfig.relaygateUrl || '';
				relaygateUrlInput.addEventListener('input', () => {
					updateApiKeyLink();
					debouncedUpdateLink();
				});
				const advancedHelp = document.createElement('small');
				advancedHelp.style.color = '#888';
				advancedHelp.style.display = 'block';
				advancedHelp.style.marginTop = '0.35em';
				advancedHelp.textContent = 'Leave empty for the official Relaygate API. Use HTTPS. Set when using self-hosted or another deployment.';
				advancedWrap.appendChild(relaygateUrlInput);
				advancedWrap.appendChild(advancedHelp);
				advancedBtn.after(advancedWrap);

				if (extraConfig.relaygateUrl) {
					advancedWrap.style.display = 'block';
				}
				advancedBtn.addEventListener('click', () => {
					const open = advancedWrap.style.display === 'none';
					advancedWrap.style.display = open ? 'block' : 'none';
				});

				const relaygateHelpText = document.createElement('small');
				relaygateHelpText.className = 'relaygate-footer-help';
				relaygateHelpText.style.color = '#888';
				relaygateHelpText.style.marginTop = '0.5em';
				relaygateHelpText.style.display = 'block';
				const raw = (extraConfig.relaygateUrl || '').trim();
				let origin = 'https://relaygate.xyz';
				if (raw) {
					try { origin = new URL(raw).origin; } catch (e) {}
				}
				const vaultUrl = origin + '/dashboard/provider-vault';
				relaygateHelpText.innerHTML = 'Use your Relaygate proxy key (not raw debrid tokens). Select the debrid services listed in your <a href="' + vaultUrl + '" target="_blank" rel="noopener noreferrer" style="color: #64ffda;">provider vault</a>.';
				configDiv.appendChild(relaygateHelpText);
			} else {
	...
	existingServices.forEach(service => {
		...
		if (service.provider === 'Usenet') {
		...
		} else if (service.provider === 'Relaygate') {
			extraConfig = {
				relaygateUrl: service.relaygateUrl || '',
				relaygateProviders: Array.isArray(service.relaygateProviders) && service.relaygateProviders.length
					? service.relaygateProviders
					: (service.relaygateProvider ? [service.relaygateProvider] : ['torbox'])
			};
			...
			extraConfig = {
				...
				httpMoviesMod: service.httpMoviesMod ?? true,
				httpMoviesLeech: service.httpMoviesLeech ?? true,
				httpAnimeFlix: service.httpAnimeFlix ?? true,
				http111477: service.http111477 ?? false,
				httpXDMovies: service.httpXDMovies ?? false,
				enableProxy: service.enableProxy || false,
				proxyUrl: service.proxyUrl || '',
				proxyPassword: service.proxyPassword || ''
			};
