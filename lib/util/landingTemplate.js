import { DEFAULT_DONATION_EMAIL } from './donationTracker.js'
import { pickVariant, DONATION_VARIANTS } from './abTestTracker.js'
...
.thanks-chip.tier-silver {
    border-color: rgba(192, 192, 192, 0.35);
    background: rgba(192, 192, 192, 0.1);
}

.thanks-chip.tier-gold {
...
async function landingTemplate(manifest, config = {}) {
    ...
    const donationButtonsBlock = `
        <div class="donation-actions">
            ...
            <p class="donation-tier-hint">⭐ $10+ and 🌟 $25+ donors get highlighted on the Wall of Thanks</p>
            ...
            <p class="donation-footnote donation-footnote-muted" id="donationStatusNote"></p>
        </div>`;

    const copyLine = variant.copy ? `\n        <p class="donation-copy">${variant.copy}</p>` : '';

    const goalBarBlock = `
        <div class="donation-progress" id="donationProgress" aria-live="polite">
            <div class="donation-progress-top">
                <span id="donationProgressText">$0 raised this month</span>
                <span id="donationRemainingText"></span>
            </div>
            <div class="donation-progress-track" role="progressbar" aria-valuemin="0" aria-valuemax="0" aria-valuenow="0" aria-label="Monthly donation progress">
                <div class="donation-progress-fill" id="donationProgressFill"></div>
            </div>
        </div>`;

    const donationHTML = `
    <section class="donation-panel" id="donationPanel" aria-labelledby="donationTitle" data-variant="${variantId}">
        <p class="donation-eyebrow">Support Sootio</p>
        <h3 class="donation-title" id="donationTitle">${variant.headline}</h3>${copyLine}
        ${goalBarBlock}
        ${variant.wallPosition === 'above' ? thanksWallBlock + donationButtonsBlock : donationButtonsBlock + thanksWallBlock}
    </section>`;
...
const createServiceRow = (provider = '${process.env.DEFAULT_DEBRID_SERVICE || 'RealDebrid'}', apiKey = '', extraConfig = {}) => {
		...
		const updateUsenetFields = () => {
			...
			if (select.value === 'HomeMedia') {
				...
				httpConfigDiv.innerHTML = \`<div style=\"font-weight: 600; margin-bottom: 0.5em; color: #64ffda; font-size: 0.9rem;\">HTTP Streaming Sources</div><div style=\"display: flex; flex-direction: column; gap: 0.5em;\"><label style=\"display: flex; align-items: center; font-size: 0.9rem; cursor: pointer;\"><input type=\"checkbox\" class=\"http-4khdhub\" checked style=\"margin-right: 8px;\">4KHDHub</label><label style=\"display: flex; align-items: center; font-size: 0.9rem; cursor: pointer;\"><input type=\"checkbox\" class=\"http-hdhub4u\" checked style=\"margin-right: 8px;\">HDHub4u</label><label style=\"display: flex; align-items: center; font-size: 0.9rem; cursor: pointer;\"><input type=\"checkbox\" class=\"http-uhdmovies\" checked style=\"margin-right: 8px;\">UHDMovies</label><label style=\"display: flex; align-items: center; font-size: 0.9rem; cursor: pointer;\"><input type=\"checkbox\" class=\"http-moviesdrive\" checked style=\"margin-right: 8px;\">MoviesDrive</label><label style=\"display: flex; align-items: center; font-size: 0.9rem; cursor: pointer;\"><input type=\"checkbox\" class=\"http-mkvcinemas\" checked style=\"margin-right: 8px;\">MKVCinemas</label><label style=\"display: flex; align-items: center; font-size: 0.9rem; cursor: pointer;\"><input type=\"checkbox\" class=\"http-mkvdrama\" checked style=\"margin-right: 8px;\">MKVDrama</label><label style=\"display: flex; align-items: center; font-size: 0.9rem; cursor: pointer;\"><input type=\"checkbox\" class=\"http-mallumv\" checked style=\"margin-right: 8px;\">MalluMv</label><label style=\"display: flex; align-items: center; font-size: 0.9rem; cursor: pointer;\"><input type=\"checkbox\" class=\"http-cinedoze\" checked style=\"margin-right: 8px;\">CineDoze</label><label style=\"display: flex; align-items: center; font-size: 0.9rem; cursor: pointer;\"><input type=\"checkbox\" class=\"http-xdmovies\" checked style=\"margin-right: 8px;\">XDMovies</label><label style=\"display: flex; align-items: center; font-size: 0.9rem; cursor: pointer;\"><input type=\"checkbox\" class=\"http-vixsrc\" checked style=\"margin-right: 8px;\">VixSrc (HLS)</label><label style=\"display: flex; align-items: center; font-size: 0.9rem; cursor: pointer;\"><input type=\"checkbox\" class=\"http-moviesmod\" checked style=\"margin-right: 8px;\">MoviesMod</label><label style=\"display: flex; align-items: center; font-size: 0.9rem; cursor: pointer;\"><input type=\"checkbox\" class=\"http-moviesleech\" checked style=\"margin-right: 8px;\">MoviesLeech</label><label style=\"display: flex; align-items: center; font-size: 0.9rem; cursor: pointer;\"><input type=\"checkbox\" class=\"http-animeflix\" checked style=\"margin-right: 8px;\">AnimeFlix</label></div>\`;
	...
	(function setupDonationPanel() {
		...
		const abVariant = donationPanel ? donationPanel.getAttribute('data-variant') : null;

		function trackAbEvent(event) {
			if (!abVariant) return;
			try {
				navigator.sendBeacon('/ab/track', new Blob([JSON.stringify({ variant: abVariant, event: event })], { type: 'application/json' }));
			} catch (e) {}
		}
		...
		if (!donationPanel || !thanksWallList || !donationAmountInput || !donatePaypalBtn) {
			return;
		}
		...
		function buildPayPalUrl(amount) {
			...
			params.set('item_name', 'Sootio donation');
		...
		function renderThanksWall(donors) {
			...
			var tierStars = { gold: '🌟', silver: '⭐' };

			donorList.forEach(function(donor) {
				var name = typeof donor === 'string' ? donor : (donor.name || 'Anonymous');
				var tier = typeof donor === 'object' && donor.tier || 'base';
				var chip = document.createElement('span');
				chip.className = 'thanks-chip' + (tier !== 'base' ? ' tier-' + tier : '');
				if (tierStars[tier]) {
					var star = document.createElement('span');
					star.className = 'thanks-star';
					star.textContent = tierStars[tier];
		...
		function renderDonationStatus(status) {
			...
			var progressEl = document.getElementById('donationProgress');
			var progressText = document.getElementById('donationProgressText');
			var remainingText = document.getElementById('donationRemainingText');
			var progressFill = document.getElementById('donationProgressFill');
			if (progressEl && status) {
				if (status.showGoalBar) {
					progressEl.classList.add('visible');
					var raisedUsd = Number(status.raisedUsd || 0);
					var goalUsd = Number(status.goalUsd || 50);
					var remainingUsd = Number(status.remainingUsd || Math.max(0, goalUsd - raisedUsd));
					var pct = Math.max(0, Math.min(100, Number(status.progressPercent || 0)));
					if (progressText) progressText.textContent = formatUsd(raisedUsd) + ' raised this month';
					if (remainingText) remainingText.textContent = remainingUsd > 0 ? (formatUsd(remainingUsd) + ' to go') : 'Goal reached!';
					if (progressFill) progressFill.style.width = pct + '%';
					var track = progressFill && progressFill.parentElement;
					if (track) {
						track.setAttribute('aria-valuenow', String(Math.min(goalUsd, raisedUsd)));
						track.setAttribute('aria-valuemax', String(goalUsd));
					}
				} else {
		...
		async function loadDonationStatus() {
			try {
				...
				// Wall of thanks will show empty state
			}
		}
		...
		if (donatePaypalBtn) {
			donatePaypalBtn.addEventListener('click', function() { trackAbEvent('click'); });
		}

		try {
			const query = new URLSearchParams(window.location.search);
			if (query.get('donation') === 'thanks' && donationStatusNote) {
				donationStatusNote.textContent = 'Thank you for supporting Sootio! Your name will appear on the Wall of Thanks shortly.';
			}
		} catch (e) {
